...
function manageCacheSize() {
    if (componentNameCache.size > 500)
        componentNameCache.clear();
    if (typeDeclarationCache.size > 1000)
        typeDeclarationCache.clear();
    if (instrumentedNodeCache.size > 3000)
        instrumentedNodeCache.clear();
    if (parentPathCache.size > 1000)
        parentPathCache.clear();
...
function getComponentName(filename, path) {
    ...
    let componentPath = path.findParent(p => p.isClassDeclaration() ||
        ...
        (p.isVariableDeclarator() &&
            p.get('init') && (p.get('init').isArrowFunctionExpression() ||
...
function isPartOfInstrumentation(path) {
    ...
    while (currentPath && depth < maxDepth) {
        ...
        if (currentPath.isMemberExpression() &&
            ...
            if (nodeId)
                parentPathCache.set(nodeId, true);
            return true;
        }
    ...
    if (nodeId)
        parentPathCache.set(nodeId, false);
    return false;
}
...
function isFlowTypeDeclaration(path, maxDepth = 3) {
    ...
    if (FLOW_TYPE_NODES.has(nodeType)) {
        if (nodeId)
            typeDeclarationCache.set(nodeId, true);
        return true;
    }
    ...
    if (nodeId)
        typeDeclarationCache.set(nodeId, false);
    return false;
}
    ...
    return {
        ...
        visitor: {
            ...
            ConditionalExpression(path, state) {
                if (!shouldInstrument(path, state))
                    return;
                ...
                path.node.test = createBranchInstrumentation(path, componentName, test, 'ternary');
                // Mark as instrumented
                markAsInstrumented(path);
            },
            IfStatement(path, state) {
                ...
                path.node.test = createBranchInstrumentation(path, componentName, test, 'if');
                // Mark as instrumented
                markAsInstrumented(path);
            },
