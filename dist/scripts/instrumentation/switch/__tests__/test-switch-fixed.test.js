...
const mockInstrumentationDetails = {
    name: 'SWITCH_INSTRUMENTATION',
    version: '1.0.0-test'
};
// Manual mock of the module
jest.mock('../test-switch-fixed.js', () => ({
    transformCode: mockTransformCode,
    instrumentationDetails: mockInstrumentationDetails
}), { virtual: true });
// Import the module after mocking
const testModule = require('../test-switch-fixed.js');
describe('switch instrumentation tests', () => {
    beforeEach(() => {
        jest.clearAllMocks();
    });
    ...
    test('instrumentation details are available', () => {
        ...
        expect(testModule.instrumentationDetails.name).toBe('SWITCH_INSTRUMENTATION');
    });
});
