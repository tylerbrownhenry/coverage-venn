...
function createAsyncFunctionWrapper(path, functionName, functionId, componentName) {
    ...
    const startTrackingCall = t.expressionStatement(t.callExpression(t.memberExpression(t.identifier('COVERAGE_TRACKER'), t.identifier('trackAsyncFunctionStart')), [
    ...
    const newBody = t.blockStatement([
        ...
        t.returnStatement(t.callExpression(t.memberExpression(t.callExpression(t.memberExpression(t.functionExpression(null, // Anonymous function
        [], // No params
        ...
        true // Make it async (always true for this wrapper)
            ...
            // Success handler
            t.arrowFunctionExpression([resultId], // Function param - the resolved value
            t.callExpression(t.memberExpression(t.identifier('COVERAGE_TRACKER'), t.identifier('trackAsyncFunctionResolved')), [
                t.stringLiteral(componentName),
                t.stringLiteral(functionName),
                t.numericLiteral(functionId),
            ...
            // Error handler
            t.arrowFunctionExpression([t.identifier('error')], // Function param - the error
            t.callExpression(t.memberExpression(t.identifier('COVERAGE_TRACKER'), t.identifier('trackAsyncFunctionRejected')), [
                t.stringLiteral(componentName),
                t.stringLiteral(functionName),
                t.numericLiteral(functionId),
                t.identifier('error')
...
function instrumentAwaitExpression(path, state) {
    ...
    const closestFunction = path.findParent((p) => p.isFunction() && (p.node.async || p.node.id?.name?.includes('async')));
    if (!closestFunction)
        return; // Not inside an async function
    // Get function name
    ...
    const trackerCall = t.callExpression(t.memberExpression(t.identifier('COVERAGE_TRACKER'), t.identifier('trackAwaitPoint')), [
        t.stringLiteral(componentName),
        t.stringLiteral(functionName),
        t.numericLiteral(functionId),
    ...
    const newNode = t.awaitExpression(t.sequenceExpression([
        trackerCall,
        originalExpression
    ]));
    // Mark as instrumented
...
function instrumentPromiseMethod(path, state, methodName) {
    ...
    // 3. new Promise()
    const isPromiseLike = (objectPath.isIdentifier({ name: 'Promise' })) ||
    ...
    const trackerCall = t.callExpression(t.memberExpression(t.identifier('COVERAGE_TRACKER'), t.identifier(`trackPromise${methodName.charAt(0).toUpperCase() + methodName.slice(1)}`)), [
        t.stringLiteral(componentName),
    ...
    const newCallExpression = t.callExpression(callee, path.node.arguments);
...
function instrumentPromiseCreation(path, state) {
    ...
    const trackerCall = t.callExpression(t.memberExpression(t.identifier('COVERAGE_TRACKER'), t.identifier('trackPromiseCreation')), [
        t.stringLiteral(componentName),
        t.numericLiteral(promiseId),
    ...
    const trackerInit = t.variableDeclaration('const', [
        t.variableDeclarator(promiseTrackerId, trackerCall)
    ]);
    // Create a new Promise constructor call with the same arguments
    const newPromiseCall = t.newExpression(t.identifier('Promise'), path.node.arguments);
    // Use the tracker to wrap and track the promise
    const trackedPromise = t.callExpression(promiseTrackerId, [newPromiseCall]);
    ...
    return {
        ...
        visitor: {
            ...
            FunctionDeclaration(path, state) {
                ...
                const newBody = createAsyncFunctionWrapper(path, functionName, functionId, componentName);
