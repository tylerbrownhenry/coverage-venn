...
const mockInstrumentationDetails = {
    name: 'ASYNC_INSTRUMENTATION',
    version: '1.0.0-test'
};
// Manual mock of the module
jest.mock('../async-plugin.js', () => ({
    transformCode: mockTransformCode,
    instrumentationDetails: mockInstrumentationDetails
}), { virtual: true });
// Import the module after mocking
const testModule = require('../async-plugin.js');
describe('async instrumentation tests', () => {
    beforeEach(() => {
        jest.clearAllMocks();
    });
    ...
    test('instrumentation details are available', () => {
        ...
        expect(testModule.instrumentationDetails.name).toBe('ASYNC_INSTRUMENTATION');
    });
});
