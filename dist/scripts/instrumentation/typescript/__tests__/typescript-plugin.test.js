...
const mockInstrumentationDetails = {
    name: 'TYPESCRIPT_INSTRUMENTATION',
    version: '1.0.0-test'
};
// Manual mock of the module
jest.mock('../typescript-plugin.js', () => ({
    transformCode: mockTransformCode,
    instrumentationDetails: mockInstrumentationDetails
}), { virtual: true });
// Import the module after mocking
const testModule = require('../typescript-plugin.js');
describe('typescript instrumentation tests', () => {
    beforeEach(() => {
        jest.clearAllMocks();
    });
    ...
    test('instrumentation details are available', () => {
        ...
        expect(testModule.instrumentationDetails.name).toBe('TYPESCRIPT_INSTRUMENTATION');
    });
});
