...
const mockInstrumentationDetails = {
    name: 'FUNCTION_INSTRUMENTATION',
    version: '1.0.0-test'
};
// Manual mock of the module
jest.mock('../test-function-fixed.js', () => ({
    transformCode: mockTransformCode,
    instrumentationDetails: mockInstrumentationDetails
}), { virtual: true });
// Import the module after mocking
const testModule = require('../test-function-fixed.js');
describe('function instrumentation tests', () => {
    beforeEach(() => {
        jest.clearAllMocks();
    });
    ...
    test('instrumentation details are available', () => {
        ...
        expect(testModule.instrumentationDetails.name).toBe('FUNCTION_INSTRUMENTATION');
    });
});
