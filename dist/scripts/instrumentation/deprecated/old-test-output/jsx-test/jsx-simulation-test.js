...
function SimpleComponent(props) {
    ...
    if (props.showHeader) {
        return createElement('div', { className: 'header' }, createElement('h1', null, 'Header'));
    }
    // Conditional rendering with ternary
    const content = props.items.length > 0
        ? createElement('ul', null, ...props.items.map((item, index) => createElement('li', { key: index }, item)))
    ...
    return createElement('div', { className: 'container' }, createElement('h2', null, props.title || 'Default Title'), content, footer);
}
