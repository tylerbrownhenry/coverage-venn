...
for (const update of importUpdates) {
    ...
    if (fs.existsSync(filePath)) {
        ...
        content = content.replace(new RegExp(`from ['"]${update.oldImport}['"]`, 'g'), `from '${update.newImport}'`);
        content = content.replace(new RegExp(`require\\(['"]${update.oldImport}['"]\\)`, 'g'), `require('${update.newImport}')`);
