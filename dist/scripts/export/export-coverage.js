...
exports.addExportControls = addExportControls;
exports.addExportToReport = addExportToReport;
 ...
 */
function addExportControls(html) {
  ...
  @media (max-width: 768px) {
    ...
    html = html.replace('// Wait for DOM to be ready', `// Export functionality
    ...
    html = html.replace('document.addEventListener(\'DOMContentLoaded\', function() {', `document.addEventListener('DOMContentLoaded', function() {
        // Setup export button
    ...
    html = html.replace('</div> <!-- End of controls -->', `  <div id="notification" style="display: none; position: fixed; bottom: 20px; right: 20px; background-color: #4CAF50; color: white; padding: 15px; border-radius: 4px; box-shadow: 0 2px 5px rgba(0,0,0,0.2); z-index: 1000;"></div>
</div> <!-- End of controls -->`);
    return html;
}
/**
 ...
 */
async function addExportToReport(htmlFilePath) {
    try {
