...
function generateSummarySection(report: Report): string {
  return `
    <div class="summary">
...
function generateCoverageTable(report: Report): string {
  return `
...
function generateMetadataSection(report: Report): string {
  return `
    <div class="meta-info">
...
function renderCoverageBar(value: number | undefined): string {
  // Handle undefined values
  const safeValue = value !== undefined ? value : 0;
  
  return `
    <div class="coverage-bar">
      <div class="coverage-value ${getCoverageClass(safeValue)}" style="width: ${safeValue}%"></div>
    </div>
    ${safeValue.toFixed(1)}%
  `;
}
