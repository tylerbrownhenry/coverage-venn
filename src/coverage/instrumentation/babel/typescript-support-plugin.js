...
function createBranchInstrumentation(path, componentName, condition, branchType) {
  if (isInstrumented(path)) return condition;
  ...
  return {
    ...
    visitor: {
      ...
      ConditionalExpression(path) {
        if (isInstrumented(path) || isTypeDeclaration(path)) return;
        ...
        try {
          ...
          const instrumentedCondition = createBranchInstrumentation(
            path, 
            componentName, 
            condition, 
            'ternary'
