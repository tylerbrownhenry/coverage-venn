...
const mockInstrumentationDetails = {
  name: 'FLOW_INSTRUMENTATION',
  version: '1.0.0-test'
};

// Manual mock of the module
jest.mock('../test-flow-simple.js', () => ({
  transformCode: mockTransformCode,
  instrumentationDetails: mockInstrumentationDetails
}), { virtual: true });

// Import the module after mocking
const testModule = require('../test-flow-simple.js');

describe('flow instrumentation tests', () => {
  beforeEach(() => {
    jest.clearAllMocks();
  });
  ...
  test('instrumentation details are available', () => {
    ...
    expect(testModule.instrumentationDetails.name).toBe('FLOW_INSTRUMENTATION');
  });
});