...
module.exports = declare((api) => {
  ...
  const createFunctionTrackingCall = (functionName, component, functionId, type) => {
    return t.callExpression(
      t.memberExpression(
        t.identifier('COVERAGE_TRACKER'),
        t.identifier(`trackFunction${type}`)
      ),
      [
  ...
  const createFunctionTrackingWrapper = (component, functionId, functionName, body) => {
    return t.blockStatement([
      ...
      t.expressionStatement(
        createFunctionTrackingCall(functionName, component, functionId, 'Start')
      ),
    ...
    visitor: {
      ...
      FunctionDeclaration(path) {
        ...
        try {
          ...
          // Add instrumentation
          path.node.body = createFunctionTrackingWrapper(
            component,
            functionId,
            functionName,
            path.node.body
      ...
      IfStatement(path) {
        ...
        try {
          ...
          path.node.test = createBranchTrackingCall(
            component,
            branchId,
            'if',
            path.node.test
      ...
      SwitchStatement(path) {
        ...
        try {
          ...
          path.node.discriminant = createSwitchTrackingCall(
            component,
            switchId,
      ...
      TryStatement(path) {
        ...
        try {
          ...
          const newTryStatement = createTryCatchTrackingWrapper(
            component,
            blockId,
            path.node.block,
            path.node.handler,
