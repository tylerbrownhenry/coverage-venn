...
const testDir = path.resolve(__dirname, '../.instrumentation-test/typescript-test');
...
// Create a new TypeScript test file
const typescriptTestContent = `/**
...
fs.writeFileSync(sourceFile, typescriptTestContent);
console.log(`TypeScript test file created at: ${sourceFile}`);

// Instrument code using our Babel plugin
...
try {
  const result = babel.transformFileSync(sourceFile, {
    ...
    plugins: [
      path.resolve(__dirname, '../src/instrumentation/babel/typescript-support-plugin.js')
    ],
...
console.log('Running instrumented TypeScript code...');
try {
  ...
  // Print summary
  execSync(`node -e "require('${coverageTrackerFile}').printSummary()"`, { 
    stdio: 'inherit'
  });
