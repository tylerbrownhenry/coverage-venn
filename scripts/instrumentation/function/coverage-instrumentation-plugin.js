...
module.exports = declare((api, options = {}) => {
  ...
  function generateFunctionId(path) {
    ...
    if (!loc) return Math.random().toString(36).substr(2, 9);
  ...
  return {
    ...
    visitor: {
      ...
      IfStatement(path) {
        ...
        const trackingExpression = api.types.callExpression(
          api.types.functionExpression(
            null,
            [],
            api.types.blockStatement([
              api.types.variableDeclaration('const', [
                api.types.variableDeclarator(
                  api.types.identifier('result'),
                  test.node
                )
              ]),
