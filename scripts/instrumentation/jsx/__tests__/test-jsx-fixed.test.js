...
const mockInstrumentationDetails = {
  name: 'JSX_INSTRUMENTATION',
  version: '1.0.0-test'
};

// Manual mock of the module
jest.mock('../test-jsx-fixed.js', () => ({
  transformCode: mockTransformCode,
  instrumentationDetails: mockInstrumentationDetails
}), { virtual: true });

// Import the module after mocking
const testModule = require('../test-jsx-fixed.js');

describe('jsx instrumentation tests', () => {
  beforeEach(() => {
    jest.clearAllMocks();
  });
  ...
  test('instrumentation details are available', () => {
    ...
    expect(testModule.instrumentationDetails.name).toBe('JSX_INSTRUMENTATION');
  });
});