...
import { addExportToReport } from '../export/export-coverage';
...
interface ComponentWithTests {
  ...
  functions?: number;
  lines?: number;
  sourceInfo?: SourceInfo; // Add sourceInfo property
}

// Types from coverage analysis
...
async function extractSourceCode(filePath: string): Promise<string | null> {
  try {
...
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>Coverage Report Export - ${timestamp}</title>
  <style>
...
<body>
  ...
  <footer>
    <p>Generated by Coverage Report Tool</p>
  </footer>
</body>
</html>`;
...
function getCoverageClass(coverage: number): string {
  if (coverage >= 80) return 'metric-high';
  if (coverage >= 50) return 'metric-medium';
  return 'metric-low';
}

// Helper function to get CSS class based on confidence value
function getConfidenceClass(confidence: number): string {
  if (confidence >= 0.7) return 'correlation-high';
  if (confidence >= 0.4) return 'correlation-medium';
  return 'correlation-low';
}

// Generate data for Venn diagram
function generateVennData(components: ComponentWithTests[]): string {
  // Create sets to track which components are in each test category
  const unitTestComponents = new Set<string>();
  const integrationTestComponents = new Set<string>();
  const utilityTestComponents = new Set<string>();
  ...
  for (const component of components) {
    ...
    if (hasUnitTest) unitTestComponents.add(component.path);
    if (hasIntegrationTest) integrationTestComponents.add(component.path);
    if (hasUtilityTest) utilityTestComponents.add(component.path);
  ...
  // Triple intersection
  const allThree = [...unitTestComponents].filter(path => 
    integrationTestComponents.has(path) && utilityTestComponents.has(path)
  ).length;
...
async function main() {
  try {
    ...
    let correlationData: ComponentWithTests[] = [];
    try {
    ...
    let coverageData: ComponentCoverage[] = [];
    try {
    ...
    if (mockCoverageData && mockCoverageData.components.length > 0) {
      ...
      mergedData = mergedData.map(component => {
        ...
        const mockComponent = mockCoverageData.components.find(
          (c: any) => c.path === component.path || path.basename(c.path) === path.basename(component.path)
        );
      ...
      try {
        ...
        if (sourceCode) {
          ...
          if (component.sourceInfo) {
            ...
            component.coveredLines = Array.from(
              { length: coveredLineCount }, 
              (_, i) => i * 2 + 1
    ...
    await addExportToReport(HTML_OUTPUT_PATH);
