...
import { IstanbulCoverageAnalyzer } from '../../../src/analyzers/istanbul';
...
async function findTestsForComponent(componentPath: string): Promise<string[]> {
...
async function runTestWithCoverage(sourceFile: string): Promise<boolean> {
 ...
 */
async function ensureMockFilesCoverage(coverageData: Record<string, any>, mockFiles: string[]): Promise<Record<string, any>> {
  ...
  for (const file of mockFiles) {
    ...
    if (updatedCoverage[file]) continue;
...
function mergeCoverageMaps(coverageFiles: string[]): Record<string, any> {
  ...
  for (const file of coverageFiles) {
    try {
      ...
      for (const [path, coverage] of Object.entries<any>(coverageData)) {
...
async function main() {
  try {
    ...
    try {
      ...
      execSync(
        'NODE_ENV=test jest --testPathPattern=__mocks__/ --coverage --collectCoverageFrom="__mocks__/src/**/*.{ts,tsx}" --no-cache --verbose',
        { stdio: 'inherit' }
      );
    } catch (error: any) {
    ...
    const mockFilesWithCoverage = coverageMap.files().filter(file => 
    ...
    if (analysis.length > 0) {
      ...
      analysis.forEach(c => {
        ...
        const color = 
          c.coverage >= 80 ? '\x1b[32m' :  // Green for good coverage
